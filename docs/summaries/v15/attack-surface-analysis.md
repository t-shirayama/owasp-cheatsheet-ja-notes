# 攻撃対象領域分析チートシート 要約

## Attribution

- Original: Attack Surface Analysis Cheat Sheet
- Source: https://cheatsheetseries.owasp.org/cheatsheets/Attack_Surface_Analysis_Cheat_Sheet.html
- Copyright: Cheat Sheets Series Team
- License: Creative Commons Attribution-ShareAlike 4.0 International (CC BY-SA 4.0)
- License URL: https://creativecommons.org/licenses/by-sa/4.0/
- Changes: Japanese summary added.
- Retrieved: 2026-05-20

## 関連ファイル

- 日本語訳: [../../translations/v15/attack-surface-analysis.md](../../translations/v15/attack-surface-analysis.md)
- 開発チェックリスト: [../../checklists/v15/attack-surface-analysis.md](../../checklists/v15/attack-surface-analysis.md)

## 概要

攻撃対象領域分析は、攻撃者が入力、接続、操作、影響を与えられる場所を把握し、変更によって増減したリスクを管理する活動である。Web/API、管理画面、ファイル処理、外部連携、キュー、バックエンド、クラウド設定、依存関係まで対象に含める。

## 要点

- 入口、出口、データフロー、信頼境界、権限境界、保存データ、機微操作を棚卸しする。
- 新機能、設定変更、インフラ変更、依存関係追加、サードパーティ連携で増えた面を記録する。
- 不要なエンドポイントや機能を削除し、管理面を分離する。
- ネットワーク到達性、認証、認可、入力検証、レート制限、監査ログ、監視を適用する。
- 分析結果を脅威モデリング、セキュリティテスト、レビュー、運用監視へつなげる。

## 実装時の注意点

- 攻撃対象領域はリリースごとに変わる。増加と削減を記録する。
- 内部向けエンドポイントや管理機能も攻撃面として扱う。
- サードパーティ連携や依存関係の追加は、新しい攻撃面としてレビューする。

## ASVS との対応

| ASVS 項目 | 関連内容 |
| --- | --- |
| V15.1 | 攻撃対象領域、入口、信頼境界、外部依存、変更差分、削減策の継続管理 |
| V2.1 | 悪用ケースと脅威モデリングへ接続する入口・データフロー分析 |

