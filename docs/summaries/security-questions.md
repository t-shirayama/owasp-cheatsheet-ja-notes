# 秘密の質問チートシート 要約

## Attribution

- Original: Choosing and Using Security Questions Cheat Sheet
- Source: https://cheatsheetseries.owasp.org/cheatsheets/Choosing_and_Using_Security_Questions_Cheat_Sheet.html
- Copyright: Cheat Sheets Series Team
- License: Creative Commons Attribution-ShareAlike 4.0 International (CC BY-SA 4.0)
- License URL: https://creativecommons.org/licenses/by-sa/4.0/
- Changes: Japanese summary added.
- Retrieved: 2026-05-20

## 関連ファイル

- 日本語訳: [../translations/security-questions.md](../translations/security-questions.md)
- 開発チェックリスト: [../checklists/security-questions.md](../checklists/security-questions.md)

## 概要

秘密の質問は推測、調査、漏えい、再利用に弱く、強い認証要素として扱うべきではありません。使う場合は限定的なリスク低減策が必要です。

## 要点

- 秘密の質問を主要な認証要素にしない。
- 推測しにくく変化しにくい質問を選ぶ。
- 回答をパスワード同様に保護する。

## 実装時の注意点

- この要約はASVS Indexでの利用を前提にした実装者向け整理です。詳細確認時は原文を参照してください。
- 関連する認証、認可、ログ、入力検証、暗号、通信保護の管理策と組み合わせて適用します。

## ASVS との対応

| ASVS 項目 | 関連内容 |
| --- | --- |
| V6.4 | 秘密の質問チートシート の主要な管理策 |

