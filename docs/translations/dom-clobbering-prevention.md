...
|    | **ガイドライン** | 説明 |
|----|------------------|------|
| \# 1 | HTML サニタイザを使用する | [link](#1-html-サニタイズ) |
| \# 2 | Content-Security Policy を使用する | [link](#2-content-security-policy) |
| \# 3 | 機微な DOM オブジェクトを freeze する | [link](#3-機微な-dom-オブジェクトの-freeze) |
| \# 4 | DOM ツリーへのすべての入力を検証する | [link](#4-dom-ツリーへのすべての入力を検証する) |
| \# 5 | 明示的な変数宣言を使用する | [link](#5-明示的な変数宣言を使用する) |
| \# 6 | グローバル変数に Document と Window を使用しない | [link](#6-グローバル変数に-document-と-window-を使用しない) |
| \# 7 | 検証前に Document の組み込み API を信頼しない | [link](#7-検証前に-document-の組み込み-api-を信頼しない) |
| \# 8 | 型チェックを強制する | [link](#8-型チェックを強制する) |
| \# 9 | strict mode を使用する | [link](#9-strict-mode-を使用する) |
| \# 10 | ブラウザの機能検出を適用する | [link](#10-ブラウザの機能検出を適用する) |
| \# 11 | 変数をローカルスコープに制限する | [link](#11-変数をローカルスコープに制限する) |
| \# 12 | 本番環境では一意の変数名を使用する | [link](#12-本番環境では一意の変数名を使用する) |
| \# 13 | カプセル化などのオブジェクト指向プログラミング技法を使用する | [link](#13-カプセル化などのオブジェクト指向プログラミング技法を使用する) |
...
Web ページの DOM ツリーにマークアップを挿入する前に、`id` 属性と `name` 属性をサニタイズします ([HTML サニタイズ](#1-html-サニタイズ)を参照してください)。
