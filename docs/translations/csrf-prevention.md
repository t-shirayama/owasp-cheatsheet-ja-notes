...
次の [JEE web filter](https://github.com/righettod/poc-csrf/blob/master/src/main/java/eu/righettod/poccsrf/filter/CSRFValidationFilter.java) は、このチートシートで説明した概念の一部に関する参考例です。これは、標準ヘッダーによる same origin 検証、double submit cookie、SameSite cookie attribute というステートレスな緩和策を実装しています。([OWASP CSRFGuard](https://github.com/aramrami/OWASP-CSRFGuard) はステートフルなアプローチを扱います。)

**注意:** これは参照サンプルに過ぎず、完全ではありません。たとえば、origin と referrer ヘッダーチェックが成功した場合に制御フローを誘導するブロックがなく、referrer ヘッダーに対する port/host/protocol レベルの検証もありません。開発者は、この参照サンプルを基に完全な緩和策を構築することが推奨されます。また、CSRF チェックが有効とみなされる前に、認証と認可メカニズムも実装すべきです。

完全なソースは [こちら](https://github.com/righettod/poc-csrf) にあり、実行可能な POC を提供します。
