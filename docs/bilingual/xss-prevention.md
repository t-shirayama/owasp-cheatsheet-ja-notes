...
Cross-Site Scripting (XSS) は、信頼できないデータがブラウザ内でコードとして解釈・実行されることで発生します。攻撃者は、アカウントなりすまし、ユーザー行動の観察、外部コンテンツの読み込み、機密データの窃取などを行えます。XSS は単一の対策で完全に解決できないため、フレームワークの自動エスケープ、コンテキスト別の出力エンコーディング、HTML サニタイズ、安全な sink、CSP などを組み合わせます。
