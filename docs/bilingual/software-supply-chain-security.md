...
複雑な SSC では、スキャン、監視、テストなどのセキュリティタスクの自動化が重要です。このような自動化は、熟練した専門家による手動レビューやその他の行動の代替ではありませんが、人手による介入では達成しにくい規模と一貫性で、脆弱性や潜在的な攻撃を検知し、場合によっては対応できます。自動化を支援するツールの種類には、SAST、DAST、SCA、コンテナイメージスキャナなどがあります。組織に価値を提供する能力が最も高い具体的なツールは、組織の特性によって大きく異なります。しかし、使用するツールの種類やベンダにかかわらず、これらのツール自体も保守され、保護され、正しく設定されなければならないことを認識することが重要です。これを怠ると、組織の SSC リスクを実際に高める可能性があり、少なくとも組織に意味のある利益をもたらせない可能性があります。最後に、これらのツールは SSCS プログラム全体の一要素にすぎないことを明確に理解しなければなりません。包括的な解決策と見なしたり、すべての脆弱性を特定するものとして依存したりすることはできません。
...
手動コードレビューは、SSC リスクを低減するための重要で比較的低コストな技法です。これらのレビューは検知統制としても抑止策としても機能します。レビューは、使用されている技術とセキュアコーディングプロセスの両方に経験を持つピアによって実施されるべきであり、ソース管理システム内でコードがマージされる前に行われるべきです [[3](#references)]。レビューでは、意図しないセキュリティ欠陥と、悪意ある目的に使われ得る意図的なコードの両方を探すべきです。レビュー結果は、必要に応じて後で確認できるよう文書化すべきです。

#### バージョン管理システムを安全に設定する

ソース管理システムの侵害または悪用は、重大な SSC リスクとして一貫して認識されています [[4,5](#references)]。強力なアクセス制御、ログ記録と監視という一般的なセキュリティベストプラクティスは、VCS を保護するための二つの方法です。git の保護ブランチやマージポリシーなど、VCS システム固有のセキュリティ機能も活用すべきです。推奨ポリシーはこの[ドキュメント](https://policies.legitify.dev/)で幅広く確認できます。SCM システムの構成管理を支援するツールもあり、たとえば [Legitify](https://github.com/Legit-Labs/legitify) は [Legit security](https://www.legitsecurity.com/) によるオープンソースツールです。Legitify は GitHub と GitLab の誤設定を検知し、ベストプラクティスの実装を支援するよう設計されています。VCS にどのようなセキュリティ統制を追加する場合でも、シークレットをこれらのシステムにコミットしてはならないことを忘れてはなりません。

#### 開発プラットフォームを保護する

IDE、開発プラグイン、および類似ツールは、開発プロセスを支援できます。しかし、すべてのソフトウェアと同様に、これらのコンポーネントにも脆弱性が存在し、攻撃経路になる可能性があります。したがって、これらのツールが安全に使用されるようにするだけでなく、基盤となるシステムを保護するための手段も講じることが重要です。開発システムにはエンドポイントセキュリティソフトウェアを導入し、それに対する脅威評価を実施すべきです [[2](#references)]。開発プロセスでは、信頼され十分に評価されたソフトウェアのみを使用すべきです。これには IDE などの「中核的な」開発ツールだけでなく、プラグインや拡張機能も含まれます。さらに、これらのツールは組織のシステムインベントリの一部として含めるべきです。
...
サードパーティサービス、製品、またはソフトウェアコンポーネントを SSC に組み込む前に、ベンダと具体的な提供物の両方について、セキュリティを十分に評価すべきです。これはオープンソースとプロプライエタリの両方の提供物に適用されます。分析の形式と範囲は、検討対象コンポーネントの重要度と性質の両方に応じて大きく異なります。コンポーネントの成熟度、セキュリティ履歴、過去の脆弱性に対するベンダの対応は、ほぼどのような場合でも有用な情報です。より大規模なベンダやサービス提供物については、[FedRAMP](https://marketplace.fedramp.gov/products)、[CSA](https://cloudsecurityalliance.org/star/registry)、または各種 ISO 標準 (ISO/IEC 27001, ISO/IEC 15408, ISO/IEC 27034) などの第三者評価や認証に対して評価されているかどうかを判断することは、有用なデータポイントになり得ますが、それだけに依存してはなりません。
...
サードパーティソフトウェア依存関係は開発プロセスを大きく加速できますが、現代のアプリケーションに関連する主要なリスクの一つでもあります。依存関係は、アプリケーションに組み込む前に慎重に選択するだけでなく、SDLC 全体を通じて慎重に監視し保守しなければなりません。これを達成するためには、ソフトウェアが利用するさまざまな依存関係を把握することが重要な第一歩です。これを促進するために SBOM を使用できます。これらの SBOM の生成と消費の両方は、できれば組織の CI/CD プロセスの一部として自動化すべきです。
...
カスタム開発コード内の潜在的なセキュリティ問題を検知するために SAST を使用することは広く使われているセキュリティ技法ですが、SSC 内の OSS コンポーネントにも使用できます [[2](#references)]。内部開発コードに SAST を使用する場合と同様に、これらのツールが誤検知と見逃しの両方を生じ得ることを認識しなければなりません。したがって、SAST の結果は手動検証なしに受け入れてはならず、プロジェクトのセキュリティを包括的に示すものとして解釈すべきではありません。ただし、その限界を理解している限り、SAST スキャンは内部開発コードと OSS コードの両方を分析する際に有用であることがあります。
...
ソフトウェア利用者の観点では、デジタル署名されたコンポーネントのみを受け入れ、ソフトウェアを利用する前に署名を検証することは、そのコンポーネントが真正であり改ざんされていないことを確認するための重要な作業です。コード署名を行う側にとっては、コード署名インフラを徹底的に堅牢化することが不可欠です。これを怠ると、コード署名システムの侵害につながり、ソフトウェア利用者を標的とするものを含むさらなる悪用につながる可能性があります。
...
SSC コンポーネントが信頼できるソースから来ており、改ざんされていないことを保証することは、SSCS の重要な部分です。[SLSA 1.0](https://slsa.dev/spec/v1.0/provenance) で「何かがどこで、いつ、どのように生成されたかを説明する、ソフトウェア成果物に関する検証可能な情報」と定義される provenance の生成と利用は、この重要な部分です。provenance は、ローカル開発システムではなくビルドプラットフォームによって生成されるべきであり、攻撃者が偽造することが非常に困難で、結果を builder に正確に結び付けるために必要なすべての詳細を含むべきです [[7](#references)]。SLSA 1.0 準拠の provenance は、[FRSCA](https://github.com/buildsec/frsca) や [Github Actions](https://github.com/slsa-framework/slsa-github-generator) などの builder を使用して生成でき、[SLSA Verifier](https://github.com/slsa-framework/slsa-verifier?tab=readme-ov-file) を使用して検証できます。
...
SSCS はソフトウェアのデプロイで終わるものではありません。デプロイ済みソフトウェアは、リスクを低減するために監視および保守されなければなりません。更新によって導入されたものか、単に新たに発見された、または公開されたものかにかかわらず、新しい脆弱性はソフトウェアシステムにおける継続的な懸念です [[4](#references)]。この監視を行う際には、全体的なアプローチを使用しなければなりません。コード依存関係、コンテナイメージ、Web サーバー、オペレーティングシステムコンポーネントは、考慮すべき項目の一例にすぎません。この監視を支援するには、システムコンポーネントの正確で最新のインベントリが重要です。さらに、安全でない構成変更を監視し、対処しなければなりません。
