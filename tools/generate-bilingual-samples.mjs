...
for (const slug of v1CoverageSlugs) {
  if (pages.some((page) => page.slug === slug)) {
    continue;
  }
  const catalogPage = sheetCatalog[slug];
  if (!catalogPage) {
    throw new Error(`Missing sheet catalog entry for V1 coverage page ${slug}`);
  }
  pages.push({
    slug,
    ...catalogPage,
...
このページは、OWASP ASVS Index に対応する公開ページです。
  ...
  for (const chapter of asvsChapters) {
    const sectionLinks = chapter.sections
      .map((section) => {
        ...
        return `### [${sectionLabel(section)}](${section.id.toLowerCase().replace('.', '-')}.md)\n\n${sectionPageLinks}`;
      })
      .join('\n\n');
...
async function writeBilingualIndex() {
  const cards = buildPageIndex()
    .map((page) => `- [${page.title}](${page.slug}.md) - ${page.asvs}`)
    .join('\n');
...
async function main() {
  ...
  const originalTargetPages = originalsOnly
    ? indexedPages
    : localSources
      ? indexedPages.filter((page) => pages.some((configured) => configured.slug === page.slug) || hasLocalTranslation(page.slug))
      : pages;
